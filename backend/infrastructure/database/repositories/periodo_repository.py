...
from domain.models.periodo import Periodo, StatusPeriodo
from infrastructure.database.sqlite_connection import get_connection, session
...
class PeriodoRepository:
    ...
    async def listar(self) -> list[Periodo]:
        async with session() as conn:
            async with conn.execute(
                "SELECT * FROM periodo ORDER BY ano DESC, mes DESC"
            ) as cursor:
